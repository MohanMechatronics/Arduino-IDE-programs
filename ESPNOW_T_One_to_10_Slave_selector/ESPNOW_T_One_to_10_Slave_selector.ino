...
esp_now_peer_info_t slave6;
esp_now_peer_info_t slave7;
esp_now_peer_info_t slave8;
esp_now_peer_info_t slave9;
esp_now_peer_info_t slave10;
...
int mac7[6] = {0xBE, 0xFF, 0x4D, 0x18, 0xE3, 0x8C}; // BE:FF:4D:18:E3:8C ESP8266 GREEN
int mac8[6] = {0x86, 0xCC, 0xA8, 0x91, 0xC6, 0xB6}; // 86:CC:A8:91:C6:B6 ESP8266 ORANGE
int mac9[6] = {0xCA, 0xC9, 0xA3, 0x13, 0x08, 0x31}; // CA:C9:A3:13:08:31 ESP8266
int mac10[6] = {0xA6, 0xCF, 0x12, 0xEE, 0xB1, 0x68}; // A6:CF:12:EE:B1:68 ESP8266 GREY
...
void ScanForSlaves() {
  ...
  memset(&slave6, 0, sizeof(slave6));
  memset(&slave7, 0, sizeof(slave7));
  memset(&slave8, 0, sizeof(slave8));
  memset(&slave9, 0, sizeof(slave9));
  memset(&slave10, 0, sizeof(slave10));
  ...
  for (int ii = 0; ii < 11; ++ii) {
    ...
    slave6.peer_addr[ii] = (uint8_t) mac6[ii];
    slave7.peer_addr[ii] = (uint8_t) mac7[ii];
    slave8.peer_addr[ii] = (uint8_t) mac8[ii];
    slave9.peer_addr[ii] = (uint8_t) mac9[ii];
    slave10.peer_addr[ii] = (uint8_t) mac10[ii];
  }
  ...
  slave6.channel = CHANNEL;
  slave6.encrypt = 0;

  slave7.channel = CHANNEL;
  slave7.encrypt = 0;

  slave8.channel = CHANNEL;
  slave8.encrypt = 0;

  slave9.channel = CHANNEL;
  slave9.encrypt = 0;

  slave10.channel = CHANNEL;
  slave10.encrypt = 0;
